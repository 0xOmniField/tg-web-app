...
import { queryState } from "@api/client";
...
export const resourcesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(queryState.fulfilled, (state, action) => {
      state.commonResources = getCommonResources(
        action.payload.player.data.local
      );
    });
  },
});

export const selectCommonResource =
  (type: ResourceType) => (state: RootState) =>
    state.automata.resources.commonResources.find(
